 ...
 **/
$.validator.addMethod( "decimalNum", function( value, element, param ) {
 ...
 * @return true/false:校验不通过为false
 **/
$.validator.addMethod( "officerCard", function( value, element, param ) {
	if(param==false)return true;
	/*如果参数值存在，则进行校验*/
	var re = /^[a-zA-Z0-9]{7,21}$/;
	var valid = (re.test(value))?true:false;
	return valid;
}, $.validator.format( "请输入正确的军官证" ));
...
$.validator.addMethod( "phone", function( value, element, param ) {
	...
	if(value==undefined || value=='') return true;
	/*如果参数值存在，则进行校验*/
	var telephoneRe = /^\d{3,4}-?\d{7,9}$/;
	var mobileRe = /^1\\d{10}$/;
	var telValid =  (telephoneRe.test(value))?true:false;
	var mobileValid =  (mobileRe.test(value))?true:false;
 ...
 * RETURNS: true/false
 */
$.validator.addMethod( "lnglat", function( value, element, param ) {
	if(!param)return true;
	if(value==undefined || value=='') return true;
	/*如果参数值存在，则进行校验*/
	if(value.indexOf(",")!=-1){
		...
		var lngRe =  /^[\-\+]?(0?\d{1,2}\.\d{1,15}|1[0-7]?\d{1}\.\d{1,15}|180\.0{1,15})$/;
		//纬度： -90.0～+90.0（整数部分为0～90，必须输入1到10位小数）
		var latRe =  /^[\-\+]?([0-8]?\d{1}\.\d{1,15}|90\.0{1,15})$/;

		var lngValid =  (lngRe.test(arr[0]))?true:false;
		var latValid =  (latRe.test(arr[1]))?true:false;
		return (lngValid & latValid);

	} else {
		return false;
	}
}, $.validator.format( "请输入正确的格式如：101.788769,36.630778" ) );
...
$.validator.addMethod( "longitude", function( value, element, param ) {
	...
	if(value==undefined || value=='') return true;
		//经度： -180.0～+180.0（整数部分为0～180，必须输入1到10位小数）
		var lngRe =  /^[\-\+]?(0?\d{1,2}\.\d{1,15}|1[0-7]?\d{1}\.\d{1,15}|180\.0{1,15})$/;
		var longitude =  (lngRe.test(value))?true:false;
		return longitude;
		
}, $.validator.format( "请输入正确的格式如：101.788769" ) );
...
$.validator.addMethod( "latitude", function( value, element, param ) {
	...
	if(value==undefined || value=='') return true;
		//纬度： -90.0～+90.0（整数部分为0～90，必须输入1到10位小数）
		var latRe =  /^[\-\+]?([0-8]?\d{1}\.\d{1,15}|90\.0{1,15})$/;
		var latitude =  (latRe.test(value))?true:false;
		return latitude;
...
/**
* FUNCTION:  校验key为数字或字符串
* PARAMETER: 字符串
* RETURNS: true/false
*/
$.validator.addMethod( "keyText", function( value, element, param ) {
	if(param==false)return true;
	/*如果参数值存在，则进行校验*/
	var empty = $.trim(value).length?false:true;
	if(empty)return true;
	...
	var valid =  (re.test(value))?true:false;		
	return valid;
}, $.validator.format( "只能包含英文、数字(长度为10)" ) );
...
* @return true/false:校验不通过为false
**/
$.validator.addMethod( "moneyNumber2", function( value, element, param ) {
	if(param==false)return true;
	/*如果参数值存在，则进行校验*/
	var empty = $.trim(value).length?false:true;
	if(empty)return true;
